...
public class Cake {
    ...
    // Method to update the maker and price of a Cake by its current maker
    public static void updateMakerAndPrice(ArrayList<Cake> cakeArray, String currentMaker, String newMaker, int newPrice) {
        for (Cake cake : cakeArray) {
            if (cake.getMaker().equals(currentMaker)) {
                cake.setMaker(newMaker);
                cake.setPrice(newPrice);
                System.out.println("Maker and price updated for " + currentMaker);
                return;
            }
        }
        System.out.println("Cake by maker " + currentMaker + " not found.");
    }
    ...
    // Method to sort Cakes by price in descending order
    public static void sortCakesByPriceDesc(ArrayList<Cake> cakeArray) {
        Collections.sort(cakeArray, Comparator.comparingInt(Cake::getPrice).reversed());
    }
